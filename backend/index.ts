...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE users SET token = $1 WHERE user_id = $2', [token, user.user_id]);
...
app.get('/user', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM users WHERE user_id = $1', [user.user_id]);
...
app.post('/orderUser', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    // Create the order
    const orderResult = await pool.query('INSERT INTO orders (user_id, delivery_address) VALUES ($1, $2) RETURNING order_id', [user.user_id, user.address]);
    const orderId = orderResult.rows[0].order_id;

    // Insert each cart item into the order_details table
    for (const item of cartItems.items) {
      await pool.query('INSERT INTO order_details (order_id, product_id, quantity) VALUES ($1, $2, $3)', [orderId, item.product_id, item.quantity]);
    }
...
app.delete('/ordersUser/:userId', async (req: Request, res: Response) => {
  try {
    ...
    await pool.query('DELETE FROM order_details WHERE order_id IN (SELECT order_id FROM orders WHERE user_id = $1)', [userId]);
